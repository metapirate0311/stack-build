...
function _AlertChangeChain() {
 window.alert("Please make sure you are in Rinkeby Testnet.");
}

function _AlertInstallMetamask() {
 window.alert("Metamask is not available, please install it.");
}

function _AlertInsufficientERC20() {
 window.alert("You don't have enough KI token, please purchase it.");
}

async function _EnableEthereum(gameObjectName, callback, fallback) {
...
function _EthereumInit(gameObjectName, callBackAccountChange, callBackChainChange) {
 const parsedObjectName = UTF8ToString(gameObjectName);
...
async function _Request(message, gameObjectName, callback, fallback) {
...
var asmLibraryArg = {
 "AlertChangeChain": _AlertChangeChain,
 "AlertInstallMetamask": _AlertInstallMetamask,
 "AlertInsufficientERC20": _AlertInsufficientERC20,
 "EnableEthereum": _EnableEthereum,
 "EthereumInit": _EthereumInit,
 "GetChainId": _GetChainId,
 "GetSelectedAddress": _GetSelectedAddress,
 "IsMetamaskAvailable": _IsMetamaskAvailable,
...
var dynCall_viiiiiiiiidiiii = Module["dynCall_viiiiiiiiidiiii"] = createExportWrapper("dynCall_viiiiiiiiidiiii");

var dynCall_viiiiiiiiidiiiii = Module["dynCall_viiiiiiiiidiiiii"] = createExportWrapper("dynCall_viiiiiiiiidiiiii");
...
if (!Object.getOwnPropertyDescriptor(Module, "webSocketState")) Module["webSocketState"] = function() {
 abort("'webSocketState' was not exported. add it to EXPORTED_RUNTIME_METHODS (see the FAQ)");
};
